...
%u = (1-tanh(x-38)).*cos(x)/2;
u = (1-tanh(x-20)).*cos(x)/2;
...
title('Pulse on the half line');

%% compute most unstable eigenvalue using eigs
...
uauto = [x uout];
save 'finite_diff_SH_half.dat' uauto '-double' '-ascii';

% auto boundary-value-problem code
uauto = [x uout D*uout D2*uout D*D2*uout];
save 'auto_SH_half.dat' uauto '-double' '-ascii';
